...
use crate::services::clipboard_store::{self as store, ClipboardItem, ListCursor};
...
/// 查询剪贴板历史（按最近使用倒序，预览形态），query 为关键字包含匹配。
...
pub async fn list_clipboard_items(
    ...
    Ok(store::list(state.db(), query.as_deref(), limit, cursor).await?)
}
